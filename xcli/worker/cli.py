...
def logs(
    ...
    """Show process logs."""
    from xcore.cli.worker_cmd import handle_worker
    handle_worker(ns(worker_subcommand="logs", target=target, lines=lines, follow=follow))
...
def beat(
    ...
    loglevel: str = typer.Option("info", "--loglevel", "-l"),
    schedule: Optional[str] = typer.Option(None, "--schedule", "-S", help="Beat schedule database file"),
) -> None:
    """Start Celery Beat scheduler."""
    from xcore.cli.worker_cmd import handle_worker
    handle_worker(ns(worker_subcommand="beat", detach=detach, loglevel=loglevel, schedule=schedule))
