...
def _start_api(args: Any) -> subprocess.Popen | None:
    ...
    srv = getattr(cfg.app, "server", None) if cfg else None

    _arg_app = getattr(args, "app", "main:app")
    _arg_host = getattr(args, "host", "0.0.0.0")
    _arg_port = getattr(args, "port", 8000)

    app_path = _arg_app if _arg_app != "main:app" else (getattr(srv, "app", _arg_app) if srv else _arg_app)
    host = _arg_host if _arg_host != "0.0.0.0" else (getattr(srv, "host", _arg_host) if srv else _arg_host)
    port = _arg_port if _arg_port != 8000 else (getattr(srv, "port", _arg_port) if srv else _arg_port)
    workers = getattr(args, "workers", None) or (getattr(srv, "workers", 1) if srv else 1)
    reload = getattr(args, "reload", False) or (getattr(srv, "reload", False) if srv else False)
    log_level = (
        args.loglevel.lower()
        if args.loglevel != "info"
        else (getattr(srv, "log_level", "info") if srv else "info")
    )
